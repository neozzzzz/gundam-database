...
export interface UseAdminListConfig<T = any> {
  tableName: string
  ...
  pageSize?: number
  selectQuery?: string
  initialFilters?: Partial<Record<keyof T | string, string | number | boolean | undefined>>
  debounceMs?: number
}
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        // 정렬 + 페이지네이션
        query = query
          .order(defaultOrderBy as string, { ascending: defaultOrderAsc })
          .range(from, to)
