...
// Admin 페이지 통합 설정 (V1.14 - 전역 포커스 색상 통일)
...
/**
 * 전역 포커스 색상 정책:
 * - 모든 검색창/입력 필드: focus:ring-0 focus:border-gray-900
 * - ring 없이 테두리만 검은색 (gray-900)
 * - AdminSearchFilter.tsx 참고
 */
...
export const ADMIN_STYLES = {
  ...
  filterSelect: 'w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-0 focus:border-gray-900 bg-white text-gray-900',
  searchInput: 'w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-0 focus:border-gray-900 bg-white text-gray-900',
  ...
  codeBadge: 'inline-flex px-2 py-1 text-xs font-mono font-semibold rounded bg-gray-100 text-gray-700',
  ...
  formInput: 'w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-0 focus:border-gray-900 bg-white text-gray-900',
  formTextarea: 'w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-0 focus:border-gray-900 min-h-[100px] bg-white text-gray-900',
  formSelect: 'w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-0 focus:border-gray-900 bg-white text-gray-900',
  formCheckbox: 'h-4 w-4 text-gray-900 focus:ring-0 focus:ring-offset-0 border-gray-300 rounded',
  
  // V1.10 폼 스타일
  input: 'w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-0 focus:border-gray-900 transition-colors bg-white text-gray-900',
  ...
  sectionTitle: 'text-lg font-semibold text-gray-900 mb-4 pb-2 border-b border-gray-200',
...
/**
 * 페이지 색상 설정에 맞춘 동적 스타일 생성
 * 기존 ADMIN_STYLES에 페이지별 색상 적용
 */
export function getAdminStyles(colorHex: string) {
  // Focus ring 색상 (RGBA 변환)
  const focusRing = hexToRgba(colorHex, 0.5)
  
  return {
    // 기존 ADMIN_STYLES 모두 상속
    ...ADMIN_STYLES,
    
    // 동적 색상 적용 (인라인 스타일용)
    getDynamicButtonStyle: (variant: 'primary' | 'danger' | 'edit' = 'primary') => {
      if (variant === 'danger') {
        return {
    ...
    // 동적 Focus ring (Tailwind 클래스와 함께 사용)
    getDynamicFocusRing: () => ({
    ...
    getHoverColor: () => {
      ...
      if (match) {
        const num = parseInt(match[1], 16)
        const r = Math.max(0, (num >> 16) - 32)
        const g = Math.max(0, ((num >> 8) & 0x00FF) - 32)
        const b = Math.max(0, (num & 0x0000FF) - 32)
...
/**
 * 페이지 설정에서 primary 색상 추출
 */
export function getPageColor(pageConfig: any): string {
  return pageConfig?.color?.primary || pageConfig?.color?.bgSolid || '#3B82F6'
}
