...
// MS-조직 관계 관리 페이지
// V1.12: useAdminList hook 적용 - 상태 관리 통합
...
export default function MsOrganizationsPage() {
  ...
  const {
    ...
    pageSize,
    deleteItem,
  } = useAdminList<MsOrganization>({
    tableName: 'ms_organizations',
    searchColumns: [],  // 클라이언트 검색 사용
    defaultOrderBy: 'created_at',
    defaultOrderAsc: false,
    pageSize: 40,
    selectQuery: `*, mobile_suit:mobile_suits(id, name_ko, model_number), organization:organizations(id, name_ko)`,
  })
  ...
  // 필터 적용
  useEffect(() => {
    setFilter('organization_id', filters.filterOrg || undefined)
    setFilter('relationship_type', filters.filterType || undefined)
  }, [filters.filterOrg, filters.filterType, setFilter])
  ...
  const filterType = (filters.relationship_type as string) || ''
        ...
        <div className={ADMIN_STYLES.filterCard}>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <select 
                value={filterType} 
                onChange={(e) => { setFilter('relationship_type', e.target.value || undefined); setCurrentPage(1) }} 
                className={ADMIN_STYLES.input}
              >
