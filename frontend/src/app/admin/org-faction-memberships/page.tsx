...
export default function OrgFactionMembershipsPage() {
  ...
  const {
    ...
    pageSize,
    deleteItem,
  } = useAdminList<OrgFactionMembership>({
    tableName: 'org_faction_memberships',
    searchColumns: [],
    defaultOrderBy: 'created_at',
    defaultOrderAsc: false,
    pageSize: 40,
    selectQuery: `*, organization:organizations(id, name_ko), faction:factions(id, name_ko, color)`,
  })
  ...
  // 필터 적용
  useEffect(() => {
    setFilter('organization_id', filters.organization_id || undefined)
    setFilter('faction_id', filters.faction_id || undefined)
    setFilter('timeline_id', filters.timeline_id || undefined)
  }, [filters.organization_id, filters.faction_id, filters.timeline_id, setFilter])
  ...
  // 필터 값 추출
  const filterOrg = (filters.organization_id as string) || ''
  const filterFaction = (filters.faction_id as string) || ''
  const filterTimeline = (filters.timeline_id as string) || ''
        ...
        <div className={ADMIN_STYLES.filterCard}>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <select 
                value={filterOrg} 
                onChange={(e) => { setFilter('organization_id', e.target.value || undefined); setCurrentPage(1) }} 
              ...
              <select 
                value={filterFaction} 
                onChange={(e) => { setFilter('faction_id', e.target.value || undefined); setCurrentPage(1) }} 
                className={ADMIN_STYLES.input}
              >
              ...
              <select 
                ...
                onChange={(e) => { setFilter('timeline_id', e.target.value || undefined); setCurrentPage(1) }} 
                className={ADMIN_STYLES.input}
              >
