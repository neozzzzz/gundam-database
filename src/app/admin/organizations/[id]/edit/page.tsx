...
export default function EditOrganizationPage() {
  ...
  const fetchParentOrgs = async () => {
    const { data } = await supabase.from('organizations').select('id, name_ko').neq('id', id).order('name_ko')
  ...
  if (loading) {
    return <AdminLoading message="조직 정보를 불러오는 중..." spinnerColor={PAGE_CONFIG.color.primary} />
  }

  return (
    <div className="min-h-screen bg-gray-50">
      <AdminFormHeader title={`조직 수정: ${formData.name_ko}`} icon={PAGE_CONFIG.icon} backHref={PAGE_CONFIG.basePath} />
