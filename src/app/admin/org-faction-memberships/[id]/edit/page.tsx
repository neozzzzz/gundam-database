...
export default function EditOrgFactionMembershipPage() {
  ...
  const fetchData = async () => {
    ...
    const { data, error } = await supabase.from('org_faction_memberships').select('*').eq('id', id).single()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('org_faction_memberships').update({
